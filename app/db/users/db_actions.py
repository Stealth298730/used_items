...
async def sign_up(username:str,password:str,db:AsyncSession):
    ...
    if user:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,detail="Такий користувач уже зареєстрований")
...
async def sign_in(username:str,password:str,db:AsyncSession)->str:
    ...
    if not user or not user.is_verify_password(password):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="Логін або пароль не вірний")
